...
let firstset = await YAML.parse(fs.readFileSync('./plugins/ql-plugin/config/ql2f18.yaml', 'utf8'));
let secondreg = "^#?(\\d+张)?"+firstset.reg+"$"

export class ql2f18 extends plugin {
  constructor() {
    super({
      name: '清凉二元图',
      dsc: 'ql2f18',
  ...
  async ql2f18(e) {
    ...
    let set = await YAML.parse(fs.readFileSync('./plugins/ql-plugin/config/ql2f18.yaml', 'utf8'));
